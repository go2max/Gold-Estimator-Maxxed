...
class BatchStore(context: Context) {
    ...
    private fun decode(json: JSONObject): BatchResult {
        ...
        val estimates = List(estimatesJson.length()) { i -> estimatesJson.getJSONObject(i).run {
            MaterialEstimate(Material.valueOf(getString("material")), getDouble("shareLow"), getDouble("shareHigh"), getDouble("densityLow"), getDouble("densityHigh"), optNullable("volumeLow"), optNullable("volumeHigh"), optNullable("weightLow"), optNullable("weightHigh"), getDouble("confidence"))
        } }
        ...
        return BatchResult(
            ...
            clusters = clusters, assignments = assignments, estimates = estimates, overallConfidence = json.getDouble("overallConfidence"),
